...
- **Anthropic Claude API**（`AnthropicClient`）を活用した自己評価・信頼度判定
- 複数情報源間の一致度計算（**OpenAI Embedding** 類似度ベース）
- クエリに対する回答の網羅度評価
...
| 2 | LLMを活用した自己評価・信頼度判定 | `confidence.py` | `LLMSelfEvaluator`が**AnthropicClient**（`generate_content` / `generate_structured`）で評価、`ConfidenceCalculator.llm_calculate()`で統合 |
| 3 | 複数情報源間の一致度計算 | `confidence.py` | `SourceAgreementCalculator`が**OpenAI Embedding**（`text-embedding-3-large`）の類似度で算出 |
...
| `LLMSelfEvaluator.__init__()` | コンストラクタ（**AnthropicClient**初期化） |
...
flowchart TB
    ...
    style LLM_EVAL fill:#000,color:#fff
    style SRC_AGR fill:#000,color:#fff
    style QRY_COV fill:#000,color:#fff
    style AGG fill:#000,color:#fff
    style HELPER_LLM fill:#000,color:#fff
    style HELPER_EMB fill:#000,color:#fff
    style ANTHROPIC fill:#000,color:#fff
    style OPENAI_EMB fill:#000,color:#fff
...
3. 必要に応じて`LLMSelfEvaluator`が**AnthropicClient**（`generate_content` / `generate_structured`）で自己評価を実行
4. 必要に応じて`SourceAgreementCalculator`が**OpenAI Embedding**（`embed_text`）でソース一致度を算出
5. 必要に応じて`QueryCoverageCalculator`が**AnthropicClient**（`generate_content`）でクエリ網羅度を評価
...
flowchart TB
    ...
    style EVAL_RESULT fill:#000,color:#fff
    style FACTORS fill:#000,color:#fff
    style SCORE fill:#000,color:#fff
    style LEVEL fill:#000,color:#fff
    style DECISION fill:#000,color:#fff
    style CC_INIT fill:#000,color:#fff
    style CC_CALC fill:#000,color:#fff
    style CC_LLM fill:#000,color:#fff
    style CC_SEARCH fill:#000,color:#fff
    style CC_TOOL fill:#000,color:#fff
    style CC_PENALTY fill:#000,color:#fff
    style CC_VALIDATE fill:#000,color:#fff
    style CC_DECIDE fill:#000,color:#fff
    style LE_INIT fill:#000,color:#fff
    style LE_EVAL fill:#000,color:#fff
    style LE_FACTORS fill:#000,color:#fff
    style SA_INIT fill:#000,color:#fff
    style SA_CALC fill:#000,color:#fff
    style SA_COS fill:#000,color:#fff
    style QC_INIT fill:#000,color:#fff
    style QC_CALC fill:#000,color:#fff
    style AG_INIT fill:#000,color:#fff
    style AG_AGG fill:#000,color:#fff
    style AG_CRIT fill:#000,color:#fff
    style F1 fill:#000,color:#fff
    style F2 fill:#000,color:#fff
    style F3 fill:#000,color:#fff
    style F4 fill:#000,color:#fff
    style F5 fill:#000,color:#fff
    style SCHEMA fill:#1a1a1a,stroke:#555
    style CALCULATOR fill:#1a1a1a,stroke:#555
    style LLM_EVALUATOR fill:#1a1a1a,stroke:#555
    style SRC_CALC fill:#1a1a1a,stroke:#555
    style QRY_CALC fill:#1a1a1a,stroke:#555
    style AGGREGATOR fill:#1a1a1a,stroke:#555
...
| ライブラリ | バージョン | 用途 |
|-----------|-----------|------|
| `anthropic` | 0.x | Anthropic APIクライアント SDK（`helper_llm` 経由） |
| `helper_llm`（内部） | — | LLMクライアント抽象化。`create_llm_client("anthropic")` → `AnthropicClient` |
| `openai` | 1.x | OpenAI APIクライアント SDK（`helper_embedding` 経由） |
| `helper_embedding`（内部） | — | Embeddingクライアント抽象化。`create_embedding_client("openai")` → `OpenAIEmbedding`（`text-embedding-3-large` / 3072次元） |
| `pydantic` | v2 | `EvaluationResult`スキーマ定義、Tool Use Structured Output |
...
| メソッド | 概要 |
|---------|------|
| `__init__(config, model_name)` | コンストラクタ（**AnthropicClient**初期化） |
...
| メソッド | 概要 |
|---------|------|
| `__init__(config)` | コンストラクタ（**OpenAI Embedding Client**初期化） |
...
LLM信頼度評価の応答スキーマ。**Anthropic Tool Use**（`generate_structured()`）で使用するPydanticモデル。`score: float` と `reason: str` の2フィールドを持ち、`generate_structured(response_schema=EvaluationResult)` が呼ばれると直接インスタンスとして返される。
...
**Anthropic Claude API**（`AnthropicClient`）による自己評価クラス。回答の確信度をLLMに評価させる。

#### コンストラクタ: `__init__`

**概要**: **AnthropicClient**（`helper_llm.create_llm_client("anthropic")` 経由）を初期化し、使用するモデル名を設定する。
...
| **Process** | 1. 設定読込（`get_config()`フォールバック）<br>2. モデル名の決定（引数 or 設定値）<br>3. `create_llm_client("anthropic", default_model=self.model_name)` による **AnthropicClient** の初期化 |
...
| **Process** | 1. EVAL_PROMPTテンプレートにパラメータを埋め込み<br>2. `llm.generate_content()` で評価を実行（`temperature=0.0`, `max_tokens=10`）。戻り値は `str` を直接返す<br>3. レスポンスから数値を抽出<br>4. 0.0-1.0の範囲にクランプ |
...
**概要**: Factorsとコンテキストを考慮した総合評価。**Anthropic `generate_structured()`**（Tool Use）で `EvaluationResult` インスタンスを直接取得する。手動JSONパースは不要。
...
| **Process** | 1. 評価プロンプトを構築（ステップ目的・出力・統計データ・評価基準を含む）<br>2. `llm.generate_structured(response_schema=EvaluationResult, model=..., max_tokens=200, temperature=0.0, system=...)` で Tool Use経由の構造化出力を実行。`EvaluationResult` インスタンスが直接返る（手動JSONパース不要）<br>3. `result.score` / `result.reason` を抽出<br>4. 例外発生時は `search_max_score` → `0.5` の順でフォールバック |
...
複数ソース間の一致度計算クラス。**OpenAI Embedding API**（`text-embedding-3-large` / 3072次元）の類似度を使用してソース間の一致度を算出する。

#### コンストラクタ: `__init__`

**概要**: **OpenAI Embedding Client**（`helper_embedding.create_embedding_client("openai")` 経由）を初期化する。
...
| **Process** | 1. 設定読込<br>2. `create_embedding_client("openai")` → **OpenAIEmbedding**（`text-embedding-3-large` / 3072次元）の初期化 |
...
| **Process** | 1. 回答が2件未満の場合は1.0（完全一致）を返す<br>2. 各回答のEmbeddingを `embedding_client.embed_text()` で取得（**OpenAI Embedding API** / 3072次元）<br>3. 全ペアのコサイン類似度を計算<br>4. 平均一致度を返す |
...
クエリ網羅度計算クラス。**Anthropic Claude API**（`AnthropicClient`）を使用して、質問に対する回答の網羅度を評価する。

#### コンストラクタ: `__init__`

**概要**: **AnthropicClient**（`helper_llm.create_llm_client("anthropic")` 経由）とモデル名を初期化する。
...
|------|------|
| **Input** | `config: Optional[GraceConfig]`, `model_name: Optional[str]` |
| **Process** | 1. 設定読込<br>2. モデル名の決定<br>3. `create_llm_client("anthropic", default_model=self.model_name)` による **AnthropicClient** の初期化 |
...
| **Process** | 1. COVERAGE_PROMPTテンプレートにパラメータを埋め込み<br>2. `llm.generate_content()` で評価を実行（`temperature=0.0`, `max_tokens=10`）。戻り値は `str` を直接返す<br>3. レスポンスから数値を抽出<br>4. 0.0-1.0の範囲にクランプ |
...
検索品質・ツール成功・ソース一致度・目標達成度の4軸で総合評価するプロンプト。`generate_structured(response_schema=EvaluationResult)`（**Tool Use**）で構造化レスポンスを要求し、`EvaluationResult` インスタンスが直接返る。
...
| 3.0 | **Anthropic マイグレーション対応**（2026-04-28）: `genai.Client()` → `create_llm_client("anthropic")` による `AnthropicClient`（`LLMSelfEvaluator`, `QueryCoverageCalculator`）。`genai.Client().models.embed_content()` → `create_embedding_client("openai")` による `OpenAIEmbedding.embed_text()`（`SourceAgreementCalculator`）。`evaluate_with_factors()` の手動JSONパース → `generate_structured()` Tool Use直接返却に簡略化。外部依存 `google-genai` を廃止し `anthropic` + `helper_llm` + `openai` + `helper_embedding` に移行。全Mermaid図をノード黒背景・白文字スタイルに統一。 |
...
flowchart LR
    ...
    subgraph HELPER_LLM_GRP["helper_llm（内部）"]
        HELPER_LLM_CLS["create_llm_client\nAnthropicClient"]
        GEN_CONTENT["generate_content()\nstr直接返却"]
        GEN_STRUCT["generate_structured()\nTool Use経由\nEvaluationResult直接返却"]
        HELPER_LLM_CLS --> GEN_CONTENT
        HELPER_LLM_CLS --> GEN_STRUCT
    end

    subgraph HELPER_EMB_GRP["helper_embedding（内部）"]
        HELPER_EMB_CLS["create_embedding_client\nOpenAIEmbedding"]
        EMB_TEXT["embed_text()\n3072次元"]
        HELPER_EMB_CLS --> EMB_TEXT
    end
    ...
    CONFIDENCE --> HELPER_LLM_CLS
    CONFIDENCE --> HELPER_EMB_CLS
    HELPER_LLM_CLS --> ANT_CLIENT
    ...
    style ANT_CLIENT fill:#000,color:#fff
    style HELPER_LLM_CLS fill:#000,color:#fff
    style GEN_CONTENT fill:#000,color:#fff
    style GEN_STRUCT fill:#000,color:#fff
    style HELPER_EMB_CLS fill:#000,color:#fff
    style EMB_TEXT fill:#000,color:#fff
    style BASEMODEL fill:#000,color:#fff
    style CONFIG fill:#000,color:#fff
    style C1 fill:#000,color:#fff
    style C2 fill:#000,color:#fff
    style ANTHROPIC_SDK fill:#1a1a1a,stroke:#555
    style HELPER_LLM_GRP fill:#1a1a1a,stroke:#555
    style HELPER_EMB_GRP fill:#1a1a1a,stroke:#555
