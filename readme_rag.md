...
- Gemini Embedding（`gemini-embedding-001`, 3072次元）によるベクトル化（※EmbeddingはGeminiを使用）
...
| 4 | Embedding生成         | `services/qdrant_service.py`              | Gemini Embedding API（gemini-embedding-001, 3072次元）                   |
...
| `AsyncAPIClient`           | Anthropic API非同期クライアント（チャンク分割/Q&A生成）（Semaphore並列制御+リトライ）     |
...
flowchart TB
    ...
    subgraph EXTERNAL["外部サービス層"]
        ANTHROPIC_LLM["Anthropic LLM API<br/>claude-sonnet-4-6"]
...
| `async_api_client.py`            | `AsyncAPIClient`    | `__init__()`                  | Anthropic APIクライアント初期化（Semaphore並列制御）          |
| `async_api_client.py`            | `AsyncAPIClient`    | `generate_content()`          | Semaphore制御付きAnthropic API呼び出し（構造化出力）          |
...
| `config.py` | `ModelConfig`        | `get_model_limits()`        | Anthropic Claudeモデルの制限を取得                |
...
flowchart TB
    ...
    subgraph CONFIG_PKG["設定"]
        CONFIG["config.py<br/>ModelConfig / DatasetConfig<br/>QdrantConfig / AnthropicConfig"]
    end
...
| モジュール                         | 用途                                |
| ---------------------------------- | ----------------------------------- |
| `chunking.async_api_client`        | Anthropic API非同期呼び出し（チャンク分割）            |
...
| メソッド                                                  | 概要                                          |
| --------------------------------------------------------- | --------------------------------------------- |
| `__init__(api_key, max_workers, max_retries, ...)`        | コンストラクタ（Anthropic接続、Semaphore初期化） |
...
| -------------------- | --------------------------- | ------------------------ | -------------------------------- |
| `text`               | str                         | -                        | 分割対象テキスト                 |
| `model`              | str                         | "claude-sonnet-4-6" | 使用するAnthropic LLMモデル         |
...
Anthropic APIへの非同期呼び出しを管理。Semaphoreで並列数を制御し、指数バックオフでリトライする。
...
**概要**: Semaphoreで並列数を制御しながらAnthropic API呼び出し。不完全JSONの検出とリトライ機能を含む。
...
│   ├── async_api_client.py           # Anthropic API非同期クライアント
