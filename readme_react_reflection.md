...
システムの特徴（ReAct + Reflection、フルスクラッチ実装、Anthropic Claude対応）です。
...
3. フルスクラッチ実装:
   　　Anthropic APIを直接利用し、柔軟な制御を実現しました。
```
...
`agent_rag.py` は、**Anthropic Claude** に対応したRAG（Retrieval-Augmented Generation）システムの統合管理ツールです。

**一言で言うと**: Anthropic Claude活用型RAG Q&A生成・Qdrant管理、および **ReAct型エージェント** による対話を実現する統合Streamlitアプリケーション
...
| エージェント対話 | 🤖       | **ReAct Agent** (Claude Sonnet) との対話。ナレッジベース検索 + **Reflection (自己推敲)** による高品質な回答。 |
| 未回答ログ       | 📊       | エージェントが回答できなかった質問のログ分析                                                               |
| RAGデータDL      | 📥       | HuggingFace/ローカルファイルからデータ取得・前処理                                                         |
| Q/A生成          | 🤖       | **Claude Sonnet** によるQ&Aペア自動生成（Celery並列処理対応）                                           |
...
graph TD
    ...
    subgraph DataAccess [データアクセス層]
        API["Gemini API (Embed: gemini-embedding-001)"]
        DB["Qdrant"]
    end
...
class API,DB,Entry,Pages,Services,Tools default
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class AgentPage,AgentSvc,HelperRag,InitUI,LogPage,LogSvc,Main,OtherPages,QS,QdrantWrapper,Tools default
...
graph TB
    ...
    Entry --> AgentUI
    AgentUI --> ReAct
    ReAct -- "Prompt + History" --> Claude
    Claude -- "Function Call" --> ReAct
    ReAct -- "Execute" --> Tools
    Tools -- "Search" --> Qdrant
    Qdrant -- "Documents" --> Tools
    Tools -- "Observation" --> ReAct
    ReAct -- "Observation" --> Claude
    Claude -- "Final Answer" --> ReAct
    ReAct --> AgentUI
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class AgentUI,Answer,Call,Claude,Entry,History,Qdrant,ReAct,Tools default
...
*   **クラス `ReActAgent`**:
    *   **セッション管理**: Anthropic API とのチャットセッションを維持。
...
* **機能**: Claude Sonnet を用いたチャットインターフェース。
* **特徴**:
...
AnthropicのTool Use機能を利用し、以下のサイクルを回します。
...
# Anthropic Claude Hybrid RAG Agent - 理論と実装リファレンス

本ドキュメントは、ReAct + Reflection エージェントの理論的背景（概念図）と、ユーザーが選択可能なClaude（Anthropic）モデルを活用した`agent_rag.py` および関連モジュールの実装詳細を体系的にまとめたリファレンスです。

---

## 第2部: アーキテクチャ概念 (Theoretical Architecture)

Claude エージェントの思考プロセスは、大きく2つのフェーズ（解決と推敲）で構成されています。
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class Action,Decision,FinalAns,Observation,Start,Thought1 default
...
Claudeモデル自体が、システムプロンプトの指示に基づき、「入力文」を解釈し、「最適な検索クエリ」へと変換（推論） しています。
...
* コード処理 (`run_agent_turn` -> `search_rag_knowledge_base`):
  Anthropic API から返ってきた tool_use 情報（モデルが決めたクエリ）を Python 側で受け取り、そのまま検索関数を実行します。
...
| 2. 変換  | Claude (LLM)       | 文脈から「検索用クエリ」を推論・生成する | 「実験生物学 実験操作」 (ケースB)       |
...
2. **推敲フェーズ** (Reflection)
   ...
   * **コード処理**: `REFLECTION_INSTRUCTION` (評価プロンプト) とドラフト回答を結合し、再度 Claude に送信します。
...
| 2. 自己評価 | `Claude (LLM)`       | 基準（正確性・スタイル）に従って評価 | 「自己評価: ...修正は不要と判断しました。」            |
| 3. 最終化   | `Claude (LLM)`       | 修正版（またはそのまま）の回答を出力 | 「社内ナレッジによると...（最終回答）」                |
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class R_Check,R_Input,R_Output,R_Reflect,R_Revise default
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class Critique,Draft,Final,Reasoning,Revise,User default
...
* **`setup_agent(selected_collections, model_name)` 関数**:
  ...
  * **詳細**: UIでユーザーが選択した `model_name` を引数として受け取り、そのモデル名を使用してLLMをセットアップします。これにより、利用するClaudeモデルを動的に切り替えることが可能です。

### ReActループの実装 (`run_agent_turn`)

Anthropic API の Tool Use 機能と Python の `while` ループを組み合わせて ReAct を実現しています。
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class Action,Critique,Decide,Draft,Finalize,Observe,Output,Review,Start,Think,User default
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class API,Config,Embed,Engine,Query,Results,Score,Search,Sparse,Vector default
style Python App fill:#1a1a1a,stroke:#fff,color:#fff
style Qdrant DB fill:#1a1a1a,stroke:#fff,color:#fff
```
...
| **Service Layer** | `services/qdrant_service.py`     | `embed_query_for_search()`  | クエリ文字列を**Dense Vector**（密ベクトル）に変換します（Gemini Embedding API）。                                                    |
...
graph TD
    ...
    AgentPage --> |API Call| Claude
    Tools --> |Search| QdrantSvc
    QdrantSvc --> |Query| QdrantDB
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class AgentPage,Call,Claude,Config,QdrantDB,QdrantSvc,Tools default
