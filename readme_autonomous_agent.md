...
| # | 責務             | 対応モジュール          | 説明                                                                    |
| - | ---------------- | ----------------------- | ----------------------------------------------------------------------- |
| 1 | 実行計画の策定   | `grace/planner.py`      | LLM（Anthropic Claude）による計画生成、複雑度推定、フォールバック計画 |
...
flowchart TB
    ...
    subgraph Planner_Layer["Planner Layer (grace/planner.py)"]
        ...
        PlanGeneration["計画生成<br/>(Anthropic Claude)"]
    end
...
class AskUser,ChatHistory,ChatInput,CollectionCache,ComplexityEstimator,ConfCalc,DB,ExecView,Executor,IntHandler,KeywordExtractor,PlanGeneration,PlanView,Planner,Qdrant,RAGSearch,Reasoning,ReplanOrch,ResultView,StateManager,ToolRegistry default
...
| モデル名                     | 説明                             |
| ---------------------------- | -------------------------------- |
| `claude-sonnet-4-6`          | Claude Sonnet 4.6（**デフォルト**） |
| `claude-opus-4-6`            | Claude Opus 4.6（最高性能）         |
| `claude-haiku-4-5-20251001`  | Claude Haiku 4.5（高速・軽量）      |
...
sequenceDiagram
    ...
    Planner->>Planner: 5. LLM計画生成 (Anthropic Claude)
    ...
    loop 各ステップ (Generator)
        ...
        alt rag_search
            ...
            Tools->>Tools: 14. LLM推論 (Claude Sonnet)
        end
...
**概要**: ユーザーの質問を分析し、LLM（Claude Sonnet）を使って ExecutionPlan を生成する。AFC（Automatic Function Calling）無効化によりJSON途切れバグを回避。
...
| **Process** | 1.`_get_available_collections()` — Qdrantから利用可能コレクション一覧を取得<br>2. `estimate_complexity_with_llm(query)` — LLMで複雑度を推定（0.0-1.0）<br>3. `PLAN_GENERATION_PROMPT` にコレクション一覧・クエリを埋め込み<br>4. Anthropic API で JSON Schema 出力（`response_schema=ExecutionPlan`, `max_output_tokens=8192`, AFC無効化）<br>5. `ExecutionPlan.model_validate_json()` でパース<br>6. LLMが推定した複雑度で `plan.complexity` を上書き<br>7. `validate_plan_dependencies()` で依存関係を検証（警告のみ）<br>8. 計画IDを付与（`create_plan_id()`） |
...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Input**   | `query: str` — ユーザーの質問文                                                                                                                                                                          |
| **Process** | `COMPLEXITY_ESTIMATION_PROMPT` を使い、Anthropic API で数値のみを取得（`temperature=0.1`, `max_output_tokens=10`, AFC無効化）。レスポンスが None の場合（AFC永続化バグ）はキーワードベース版にフォールバック |
...
| **Process** | 元の計画情報とフィードバックをプロンプトに含め、Anthropic API で JSON Schema 出力（AFC無効化）。新しいplan_idを付与 |
...
| `config.ModelConfig`        | Anthropic Claudeモデル設定（利用可能モデル一覧）                         |
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class API,Claude,ConfMod,Config,ExecutorMod,Init,IntervMod,MeCab,Page,PlannerMod,Qdrant,QdrantConfig,ReplanMod,Schemas,StepStatus,ToolsMod,create_planner default
style UI fill:#1a1a1a,stroke:#fff,color:#fff
...
| `ReasoningTool` | `reasoning`  | LLM推論（検索結果を基にClaudeで回答生成）                          |
...
| `LLMSelfEvaluator`          | Claude による自己評価（query と answer の整合性）                                                 |
...
### B.2 ModelConfig（Anthropic LLM）


| 設定項目              | デフォルト値               | 説明             |
| --------------------- | -------------------------- | ---------------- |
| `DEFAULT_MODEL`       | `"claude-sonnet-4-6"` | デフォルトモデル |
...
| 設定項目     | デフォルト値             | 説明                 |
| ------------ | ------------------------ | -------------------- |
| `provider`   | `"gemini"`               | 埋め込みプロバイダー（Gemini Embeddingを使用） |
...
| Planner/Executor 初期化エラー            | APIキー未設定                      | `ANTHROPIC_API_KEY` または `GOOGLE_API_KEY` を設定                                                                       |
| GraceConfig 読み込みエラー               | YAML ファイル不在 or 構文エラー    | `config/grace_config.yml` を確認。不在時はデフォルト値で動作                                                          |
| 計画策定が常にフォールバック             | LLM呼び出し失敗またはAFC永続化バグ | API認証・ネットワーク・モデル名を確認。AFC無効化は実装済み（anthropic issue #1818）。ログ`logs/grace_run.log` 参照 |
...
| LLM応答がNone                            | AFC永続化バグ（anthropic）      | `automatic_function_calling=AutomaticFunctionCallingConfig(disable=True)` が設定されていることを確認                  |
