## (1) 自律型Agent 
### 計画策定（Plan）→ 実行（Execute,実行、スコア）→ 信頼度評価（Confidence）
### → 介入判定（Intervention）→ リプラン（Replan））
![自律型Agent](assets/lp_img.png)
## (2) RAGデータのHuggingFaceからダウンロード、Qdrant(ベクターDBへ)登録
![RagデータDL・登録](assets/qa_pair_img.png)

## Agent RAG (Anthropic) プロジェクト
...
画面： agent_rag.py
![readme_img.png](assets/readme_img.png)
...
**概要**: ユーザーの質問を分析し、LLM（Claude Sonnet）を使って ExecutionPlan を生成する。
...
| **Process** | 1.`_get_available_collections()` — Qdrantから利用可能コレクション一覧を取得<br>2. `estimate_complexity_with_llm(query)` — LLMで複雑度を推定（0.0-1.0）<br>3. `PLAN_GENERATION_PROMPT` にコレクション一覧・クエリを埋め込み<br>4. Anthropic API で JSON Schema 出力（`response_schema=ExecutionPlan`）<br>5. `ExecutionPlan.model_validate_json()` でパース<br>6. `validate_plan_dependencies()` で依存関係を検証<br>7. 計画IDを付与（`create_plan_id()`） |
...
| ----------- | ------------------------------------------------------------------------------------------------------- |
| **Input**   | `query: str` — ユーザーの質問文                                                                        |
| **Process** | `COMPLEXITY_ESTIMATION_PROMPT` を使い、Anthropic API で数値のみを取得（temperature=0.1）                   |
...
| Planner/Executor 初期化エラー            | APIキー未設定                   | `ANTHROPIC_API_KEY`（LLM用）または `GOOGLE_API_KEY`（Gemini Embedding用）を設定                      |
