...
uv run python -m chunking.csv_text_to_chunks_text_csv \
  --input-file OUTPUT/cc_news_3per.csv \
...
uv run python qa_qdrant/make_qa_register_qdrant.py \
  --input-file output_chunked/cc_news_3per_chunks.csv \
  --collection cc_news_3per \
  --model claude-sonnet-4-6 \
  --concurrency 8 \
  --recreate
...
# CSVファイル → チャンクCSV
# 走行時にエラーが出た場合は：
# Anthropic はレート制限（429）や過負荷（529）が発生することがあります。
# その場合は --workers を減らすか、block_size を小さくしてください。

# 出力例:
# chunks_output/wikipedia_ja_5per_chunks.csv        （メタデータ付き・固定ファイル名）
# chunks_output/wikipedia_ja_5per_chunks_simple.csv （シンプル版・固定ファイル名）
# タイムスタンプを付ける場合は --timestamp オプションを追加
...
python -m chunking.csv_text_to_chunks_text_csv \
  --input-file ./data/document.txt
  # → chunks_output/document_chunks.csv が生成される（固定ファイル名）
  # → chunks_output/document_chunks_simple.csv （シンプル版）も同時生成
  # タイムスタンプ付きにする場合: --timestamp を追加
"""
